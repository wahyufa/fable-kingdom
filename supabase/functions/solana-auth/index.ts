// Fable Kingdom — Solana wallet auth
// Verifies a "sign in with Solana" signature, then mints a Supabase session
// JWT directly. The user is stored with a synthetic email of
// "<wallet>@wallet.fablekingdom" so they slot into Supabase auth + RLS exactly
// like an email user.
//
// Deploy:   supabase functions deploy solana-auth --no-verify-jwt
// Env vars: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY (auto-injected)
//           JWT_SECRET — set this yourself with the value from
//            Project Settings → API → JWT Settings → "JWT Secret".
//            Supabase reserves the SUPABASE_ prefix, so we use plain JWT_SECRET.
//
...
function base58decode(s: string): Uint8Array {
  if (!/^[1-9A-HJ-NP-Za-km-z]+$/.test(s)) throw new Error("invalid base58");
  let n = 0n;
  ...
  const out = new Uint8Array(leading + hex.length / 2);
  for (let i = 0; i < hex.length; i += 2) {
    out[leading + i / 2] = parseInt(hex.substr(i, 2), 16);
  }
  return out;
}
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      pkBytes = base58decode(wallet);
      sigBytes = Uint8Array.from(atob(signature), (c) => c.charCodeAt(0));
    } catch (err) {
      console.error("decode failed", err);
      return bad("Malformed wallet or signature");
    }
    if (pkBytes.length !== 32 || sigBytes.length !== 64) {
      return bad("Wrong key/signature length");
    }
    ...
    if (!message.includes(wallet)) return bad("Message must include the wallet address");
    ...
    const email = `${wallet.toLowerCase()}@wallet.fablekingdom.app`;
    const cleanName = (displayName || "").trim().slice(0, 16);
    const defaultName = `${wallet.slice(0, 4)}…${wallet.slice(-4)}`;
    ...
    const { data: createData, error: createErr } = await admin.auth.admin.createUser({
      email,
      email_confirm: true,
      user_metadata: { wallet_address: wallet, display_name: cleanName || defaultName },
    });
    ...
    console.error("Unhandled error in solana-auth:", err);
