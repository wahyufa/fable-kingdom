...
### Per-mode background music + Survival upgrade caps (v42)
- [x] **Mode-specific BGM with crossfade**: single `bgm.mp3` replaced with a small track registry (`default` = Story/menus, `adventure`, `survival` — user-supplied Suno tracks copied into `assets/audio/bgm_adventure.mp3` / `bgm_survival.mp3`). One persistent `Audio` element whose `.src` swaps via `setBgmTrack(key)`: fades out (300ms) → swaps source → fades in (500ms) via a `setInterval` volume ramp, rather than a hard cut. Wired into all three mode starters (`startAdventure`/`startSurvival`/`startCampaign`); no-ops if already on that track
- [x] **Survival upgrade caps** (user report: stacking Swift Boots at high waves made movement "too fast to control" — genuinely harder to play, not just easier): every stacking upgrade used to compound with zero ceiling (`speed *= 1.12` and `atkDur *= 0.8` every pick, forever). Added `UPGRADE_CAPS` relative to newly-named `BASE_SPEED/BASE_DMG/BASE_MAX_HP/BASE_ATK_DUR` constants: speed capped at 1.6x base (368, was unboundedly compounding — 20 picks would've reached ~2216), attack duration floored at 0.4x base (2.5x attack-speed ceiling), damage capped at 125, max HP capped at 250. `applyUpgrade` clamps on every application (safe even if a maxed stat is picked again — clamps to itself, true no-op); `offerUpgrades` now filters maxed-out picks out of the random pool first and only pads them back in (clearly labeled "(MAX)" / "Already maxed") if fewer than 3 live options remain, so a long run never gets stuck offering dead buttons
- [x] Debug hooks added for both: `__ts.bgmTrack`, and `__ts.applyUpgrade`/`__ts.isUpgradeMaxed`/`__ts.upgradeCaps` for console-testing cap behavior directly
- [x] Verified end-to-end: bgm `.src` and `bgmTrack` correctly swap on every tested transition (Adventure→Survival→Adventure→Story, each landing on the right file); grinding each of the 4 stats 20x via the debug hook confirmed exact clamping (speed hit exactly 368, atkDur exactly 0.133, dmg exactly 125, maxHp exactly 250 — zero overshoot); triggering a wave-clear after maxing everything but Feast showed the real offer screen correctly serving Feast live + two "(MAX)" fallback picks instead of broken/empty buttons; full reload regression on Adventure clean, no console errors throughout
- Known minor nuance (not fixed, low severity): on the very first PLAY click, `initAudio()`'s gesture-unlock plays the default track for a fraction of a second before the mode starter's `setBgmTrack` fades it to the correct one — audible as a very brief blip, not on subsequent mode switches. Not worth the added coupling to eliminate.
